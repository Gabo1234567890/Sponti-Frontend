...
class CreateAccountViewModel(private val authRepository: AuthRepository = AuthRepository()) :
    ViewModel() {
    ...
    fun onUsernameChange(value: String) {
        _state.value = _state.value.copy(
            username = value, usernameError = null
        )
    }
    ...
    fun submit(onSuccess: () -> Unit) {
        ...
        when {
            s.username.isBlank() -> {
                _state.value = s.copy(usernameError = FieldError.Empty)
                return
            }
        ...
        viewModelScope.launch {
            ...
            try {
                val resp = authRepository.signup(
                    s.username.trim(), s.email.trim(), s.password.trim()
                )
                ...
                if (resp.isSuccessful) {
                    ...
                    when {
                        parsed?.getMessage()?.lowercase()
                            ...
                            ?.contains("username") == true -> _state.value = _state.value.copy(
                            usernameError = parsed.toFieldError()
                        )
